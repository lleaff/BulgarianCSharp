namespace System
{
	internal class ConsoleOutput
	{
		...
		public static void PrintSuperposed(string string1, string string2, int string1SubElementLength = 0, int string2SubElementLength = Int32.MinValue)
		{
			...
			for (int i = 0, j = 0, iStarting = 0, iEnding = 0, jStarting = 0, jEnding = 0, bW = Console.BufferWidth, nL = string2.Length; i < nL - 1 || j < nL - 1; )
			{
				for (j = jStarting; !((i % (int)largerElement == 0 || !(i % s1L == 0)) && i - iStarting >= bW - (int)largerElement)  && (i < nL || i < nL);)
				{
				...
				iEnding = i;
				Console.WriteLine();
				for (i = iStarting; !((j % (int)largerElement == 0 || !(j % s2L == 0)) && j - jStarting >= bW - (int)largerElement)  && (j < nL || i < nL); )
				{
